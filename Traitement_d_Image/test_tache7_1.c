...
assign_points(P5 , &cont1[1]);

bezier2 B; 
approx_bezier2(cont1 , 0 , 1, &B);
display_bezier2(B , 1);
...
bezier2 B2; 
approx_bezier2(cont2 , 0 , 2, &B2);
display_bezier2(B2 , 2);
...
bezier2 B3; 
approx_bezier2(cont3 , 0 , 4, &B3);
display_bezier2(B3 , 3);
...
bezier2 B4; 
approx_bezier2(cont4 , 0 , 8, &B4);
display_bezier2(B4, 4);
...
*/
   ...
   if (argc < 3) {
    ...
    sequences_listB2* simplified_contours=create_sequences_list_b2(); 

    //simplification bezier2
    simplification_bezier2(contours , d, simplified_contours);

    unsigned long long summ_bezier2=0;
    display_sequences_list_b2(*simplified_contours , &summ_bezier2);
    ...
    create_eps_bezier2(argv[2], H, L, *simplified_contours, 'F', d);

    //suppression des listes des contours , et contour simplifee
    destroy_sequences_list(&contours);
    destroy_sequences_list_b2(simplified_contours);
